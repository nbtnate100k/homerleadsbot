    ...
    for it in raw or []:
        ...
        if kind == "bin":
            ...
            try:
                ...
                amin, amax = None, None
            if not any([st, ct, bk, bn, br, zp, gen, amin is not None, amax is not None]):
                continue
            out.append(
                {
                    "kind": "filter",
...
def _filter_cart_entry_same(a: dict, b: dict) -> bool:
    return (
        ...
        and (a.get("age_min")) == (b.get("age_min"))
        and (a.get("age_max")) == (b.get("age_max"))
    )
...
def _has_any_buy_filter(flt: dict | None) -> bool:
    if not flt:
        return False
    if flt.get("age_min") is not None or flt.get("age_max") is not None:
        return True
    return any(
...
def _active_buy_filter_kwargs(flt: dict | None) -> dict:
    ...
    if amin is None and amax is None:
        pass
    elif amin is not None and amax is None:
        amax = 120
    elif amin is None and amax is not None:
        amin = 0
    return {
        "state": st,
        "bin6": bn,
...
def _age_stats_html_for_filters(flt: dict | None) -> str:
    """Human-readable agify age range for current filters (HTML sync tags)."""
    fk = _active_buy_filter_kwargs(flt)
    total, tagged, mn, mx, avg = matching_age_stats_combined(**fk)
    if total < 1:
        return ""
    if tagged < 1:
        return (
            "\n<i>Ages (agify): no age tags in matching stock — re-sync from the HTML BIN tool.</i>"
        )
    if mn is None or mx is None or avg is None:
        return ""
    return (
        f"\n<i>Ages (agify, tagged lines): <b>{mn}–{mx}</b> · avg <b>{avg:.1f}</b> "
        f"· <b>{tagged}</b>/<b>{total}</b> with age tag</i>"
    )
...
def _buy_filters_summary_html(flt: dict | None) -> str:
    ...
    try:
        amin = int(flt["age_min"]) if flt.get("age_min") is not None else None
        amax = int(flt["age_max"]) if flt.get("age_max") is not None else None
    except (TypeError, ValueError):
        amin, amax = None, None
    if amin is not None or amax is not None:
        lo = amin if amin is not None else 0
        hi = amax if amax is not None else 120
        parts.append(f"Age: <b>{lo}–{hi}</b> (agify tag)")
    if not parts:
        return "Current filters: <i>(none)</i>"
...
def buy_filters_grid_text(qty: int, flt: dict | None) -> str:
    return (
        ...
        "(from stock). Add <b>Gender</b> and <b>Age</b> (agify <code>|§a§</code> tags from HTML sync). "
        "Then <b>Done</b> for random checkout, or <b>Add filtered to cart</b>.</i>"
    )


def buy_age_pick_keyboard() -> InlineKeyboardMarkup:
    """Preset agify age ranges (|§a§ on synced lines)."""
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton("18–25", callback_data="bfa:18:25"),
                InlineKeyboardButton("26–35", callback_data="bfa:26:35"),
                InlineKeyboardButton("36–45", callback_data="bfa:36:45"),
            ],
            [
                InlineKeyboardButton("46–55", callback_data="bfa:46:55"),
                InlineKeyboardButton("56–65", callback_data="bfa:56:65"),
                InlineKeyboardButton("66+", callback_data="bfa:66:120"),
            ],
            [
                InlineKeyboardButton("✖️ Clear age", callback_data="bfa:clear"),
    ...
    return InlineKeyboardMarkup(
        [
            ...
            [
                ...
                InlineKeyboardButton("🎂 Age", callback_data="bf_age"),
            ],
...
def buy_flow_help_text() -> str:
    return (
        ...
        "<b>Gender</b> / <b>Age</b> (tags from HTML sync: <code>|§g§</code> / <code>|§a§</code>). "
...
def format_cart_screen(user_id: int) -> str:
    ...
    for i, it in enumerate(cart, 1):
        ...
        if it.get("kind") == "filter":
            ...
            if it.get("age_min") is not None or it.get("age_max") is not None:
                try:
                    lo = int(it["age_min"]) if it.get("age_min") is not None else 0
                    hi = int(it["age_max"]) if it.get("age_max") is not None else 120
...
async def request_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        ...
        "after HTML sync (<code>|§g§</code> + optional <code>|§a§</code> age from agify.io).\n\n"
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "bf_acart":
        ...
        if not _has_any_buy_filter(flt):
            await query.answer(
                "Pick Brand, ZIP, State, City, BIN, Bank, Gender, or Age first.",
                show_alert=True,
            )
            return
        context.user_data.pop("awaiting_cart_filter_qty", None)
    ...
    if m_bfg:
        ...
        age_block = _age_stats_html_for_filters(flt)
        await query.edit_message_text(
            f"<b>Gender</b>: {lbl}\n"
            f"Lines: <b>{tot}</b> (1st {n_first} · 2nd {n_second})"
            f"{age_block}\n\n"
            "<i>Only matches leads synced from the HTML BIN tool with a gender tag. "
            "Use Filter menu for <b>Done</b> (random buy) or add to cart below.</i>",
            parse_mode=ParseMode.HTML,
            reply_markup=InlineKeyboardMarkup(
                [
                    [
    ...
    m_bfa = re.match(r"^bfa:(clear|(\d+):(\d+))$", data)
    if m_bfa:
        flt = dict(context.user_data.get("buy_filters") or {})
        if m_bfa.group(1) == "clear":
            flt.pop("age_min", None)
            flt.pop("age_max", None)
        else:
            flt["age_min"] = int(m_bfa.group(2))
            flt["age_max"] = int(m_bfa.group(3))
        context.user_data["buy_filters"] = flt
        qty = int(context.user_data.get("buy_qty") or 0)
        if m_bfa.group(1) == "clear":
            context.user_data["buy_filters_mode"] = True
            ...
            await query.answer("Age filter cleared.")
            return
        fk = _active_buy_filter_kwargs(flt)
        n_first = count_matching_lines("first", **fk)
        n_second = count_matching_lines("second", **fk)
        tot = n_first + n_second
        lo, hi = int(m_bfa.group(2)), int(m_bfa.group(3))
        age_block = _age_stats_html_for_filters(flt)
        await query.edit_message_text(
            f"<b>Age</b>: <b>{lo}–{hi}</b> (agify)\n"
            f"Lines: <b>{tot}</b> (1st {n_first} · 2nd {n_second})"
            f"{age_block}\n\n"
            "<i>Matches <code>|§a§</code> age tags from HTML sync. "
            "Use Filter menu for <b>Done</b> or add to cart.</i>",
            parse_mode=ParseMode.HTML,
            reply_markup=InlineKeyboardMarkup(
                [
                    [
        ...
        await query.answer(f"Age {lo}–{hi}.")
        return

    if data == "bf_gender":
        await query.edit_message_text(
            "⚧️ <b>Gender</b>\n\n"
            "<i>Matches <code>|§g§male</code> / <code>|§g§female</code> on leads synced "
            "from the HTML BIN tool (genderize.io + agify.io age tag <code>|§a§</code>). "
            "Untagged lines do not match.</i>",
            parse_mode=ParseMode.HTML,
            reply_markup=InlineKeyboardMarkup(
                [
                    [
    ...
    if data == "bf_age":
        await query.edit_message_text(
            "🎂 <b>Age</b> (agify)\n\n"
            "<i>Pick a range. Matches predicted ages on <code>|§a§</code> tags from HTML sync.</i>",
            parse_mode=ParseMode.HTML,
            reply_markup=buy_age_pick_keyboard(),
        )
        return
